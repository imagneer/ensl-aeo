...
**1) why-box — 헤드라인 1개 + 불릿 2~3개**
- **헤드라인(첫 번째 문장)**: 반드시 20~40자(공백 포함, 마침표 포함) 안에 들어와야 한다. **이 범위를 넘기면 무조건 실패다.**
  - **아래 템플릿의 빈칸만 채워라. 템플릿 구조 자체를 바꾸지 마라** — 절을 두 개 다 완전한 문장으로 풀어쓰면 길어져서 반드시 실패한다.
    ${
      reasonStatedCount > 0
        ? '템플릿: "이 특징, 인지엔 있었는데 추천 근거로도 이어졌어요." (그대로 써도 됨 — 이 문장이 정확히 29자다)'
        : '템플릿: "이 특징, 인지엔 있었지만 추천 근거로는 아직이에요." (그대로 써도 됨 — 이 문장이 정확히 29자다)'
    }
  - **특징 이름을 헤드라인에 다시 쓰지 마라** — "이 특징"으로만 지칭해라. 특징 전체 이름을 쓰면 길이가 초과된다.
  - "선택 이유"라는 표현 금지 — "추천 근거"로 써라. "선택","골랐다","선호한다" 같은 AI 내부 인과 언어도 금지.
  - 제출 전에 스스로 글자 수를 세어라. 40자를 넘으면 그 문장은 쓰지 말고 위 템플릿 그대로 제출해라.
- **불릿(두 번째 문장부터, 2~3개)**: 각각 **한 줄짜리 관찰 사실**만 써라. 설명을 풀어쓰지 말고 "~됨"/"~없음"/"~반복됨" 같은 명사형 종결로 짧게 끊어라.
  예시 형식(문구 그대로 베끼지 말 것): "○○와 'X'는 인지 답변에서 직접 연결됨" / "추천 답변에서는 적합성·장점으로 연결된 문장이 없음" / "경쟁 브랜드에는 근거 연결 구조가 반복됨"
  - 위 근거 연결 사례·동시 언급 사례에서 실제로 관찰된 것만 써라. 없는 사실을 만들어내지 마라.
  - 여기도 "선택", "골랐다", "확신 못해서" 같은 금지 언어는 그대로 적용된다.
          ...
          input_schema: {
            ...
            properties: {
              why_box_sentences: {
                ...
                items: { type: 'string' },
                description:
                  '배열의 [0]번째는 헤드라인(20~40자, 짧고 대비 선명한 문장) 하나, [1]번째부터는 불릿(한 줄짜리 관찰 사실) 2~3개.',
              },
              counter_box_cases: {
                type: 'array',
                items: {
...
why-box 헤드라인(첫 문장): ${draft.whyBoxSentences[0] ?? '(없음)'}
why-box 불릿(나머지 문장):
${draft.whyBoxSentences.slice(1).map((s) => `- ${s}`).join('\n') || '(없음)'}

counter-box 설명:
${counterText || '(없음)'}

**탈락 기준(하나라도 걸리면 불합격)**:
1. why-box나 counter-box 설명에 "선택", "골랐다", "선호한다" 같은 AI 내부 인과 언어가 있는가? ("선택 이유"라는 표현도 포함 — "추천 근거"로 써야 한다)
...
4. **헤드라인이 40자를 크게 넘거나, 여러 절로 이어진 설명문처럼 장황한가?** 헤드라인은 반드시 짧고 대비가 선명한 한 문장이어야 한다 — 길고 설명적이면 그 자체로 불합격.
5. 불릿이 한 줄짜리 관찰 사실이 아니라 문단처럼 길게 풀어써졌는가?
