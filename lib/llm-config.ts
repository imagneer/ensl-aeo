 ...
 *
 * ⚠️ 이 값은 "최근 실측 평균"이 아니라 **역산 추정치 + 실측 1건**이다 —
 * lib/llm-usage.ts의 구조화 로그는 Vercel 로그 스트림에만 남고 조회
 * 가능한 곳(DB 등)에 아직 저장 안 해서(작업지시서의 llm_calls 표는
 * 이번엔 선택 항목이라 스킵함, 2026-09-04 코난 판단), "최근 평균"을
 * 코드가 직접 계산할 방법이 없다.
 *
 * 2026-09-04 크레딧 재충전 후 usage 로깅을 실제 API 호출 1건으로
 * 재검증하면서(문단 1개짜리 최소 크기 호출) input_tokens=1405가 실측됨 —
 * 처음에 9/3 사고 조사 때 역산했던 500~3,300자 프롬프트 → "평균 약 900
 * 입력 토큰" 추정이 tool_choice 스키마 정의 오버헤드를 과소평가했던 것으로
 * 보인다. 이 실측 1건(n=1, 문단 1개 기준)을 반영해 1200으로 올려 잡는다 —
 * 실제 백필 호출은 문단이 여러 개인 경우가 많아 이보다 더 클 수 있다.
 * llm_calls 표가 생기면 진짜 평균으로 교체할 것 — 그 전까지 dry-run
 * 추정치는 "대략의 자릿수"로만 믿을 것.
 */
export const ESTIMATED_TOKENS_PER_KEYWORD_CALL = { input: 1200, output: 150 } as const;
