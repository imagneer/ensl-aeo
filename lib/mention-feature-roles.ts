...
- **일반적인 품질 표현을 특징명의 더 강한 단어로 확대 해석하지 마라 (2026-09-09 발견)**. "정밀하다", "신뢰할 수 있다", "전문적이다" 같은 흔한 칭찬 표현이 있다고 해서, 특징명 안의 더 세고 구체적인 단어(예: "고난도")가 실제로 뒷받침된다고 보면 안 된다. 예: 원문이 "임플란트를 정밀하게 진료합니다"이고 특징명이 "고난도 임플란트 시술"이면, 원문에 "고난도"·"난이도"·"어려운 케이스" 같은 말이 따로 없는 한 그 특징은 등장하지 않은 것으로 본다(reason_stated는 물론 co_mentioned도 아님).
- **AI가 뭉뚱그려 말한 걸 특징명이 더 구체적으로 채워 넣고 있으면 reason_stated로 인정하지 마라 (2026-09-09 발견)**. 예: 원문이 "전문의 협진"이라고만 했는데 특징명이 "여러 과 전문의가 함께 진료"처럼 구체적인 과 이름까지 특정하고 있다면, 원문에 실제로 여러 과 이름이 나열돼 있지 않은 한 co_mentioned로 낮춰라. 마찬가지로 원문이 "통증을 최소화합니다"라고만 했는데 특징명이 "수면진정 등" 구체적인 방법까지 담고 있다면, 원문에 "수면마취"·"무통마취 장비" 같은 구체적인 방법이 실제로 나와 있지 않은 한 co_mentioned다.
